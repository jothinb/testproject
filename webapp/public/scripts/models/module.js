/*
 * Define Angular app.services module
 */
define(['angular'], function (angular) {
	'use strict';
	var module = angular.module('app.models', []);

	module.config(['$provide', function ($provide) {
		module._service = module.service;
		module._factory = module.factory;
		module._value = module.value;

		module.service = function (name, constructor) {
			$provide.service(name, constructor);
			return this;

		};

		module.factory = function (name, factory) {
			$provide.factory(name, factory);
			return this;
		};

		module.value = function (name, value) {
			$provide.value(name, value);
			return this;
		};
	}]);

/*    module.controller('YardMainCtrl', function($scope, $http) {
        $http.get("http://' + oasisHost + ':' + oasisPort + '/oasis/location")
            .success(function(response) {$scope.oasisLocations = response;});
    }) */

	return module;
});
